...
?>
<h3><b>Fill the Following Form To Add Image</b></h3>
<br />
<div class="row">
    <div class="col-md-6">
        <h6>Image Name *</h6>
        <div class="p-2">
            <input type="text" class="form-control" id="title" placeholder="Enter Image name" required>
        </div>
    </div>
    <div class="col-md-6">
        <h6>Main Category *</h6>
        <div class="p-2">
            <select class="form-control" id="maincategory" required onchange="getSubCategory(this.value)">
    ...
    $(".saveButton").click(function() {
        ...
        if (images.length == 0) {
            ...
            swal({
                ...
                text: "The Image will be saved to the server!",
                icon: 'warning',
                buttons: true,
                dangerMode: true,
            }).then((willDelete) => {
                if (willDelete) {
                    ...
                    formData.append("mode", "addImage");
                    formData.append("title", title);
                    formData.append("image", images[0]);
                    formData.append("categoryid", $("#maincategory").val());
                    formData.append("subcategoryid", $("#subcategory").val());
                    $(".preloader").show();
                    $.ajax({
                        url: "<?= $apiUrl ?>",
                        type: "POST",
